"""Procedurally generate a greyscale terrain heightmap PNG.
...
# Not exposed on the CLI — depth is the one crater knob worth tuning per world.
...
def generate(size: int, seed: int, octaves: int, persistence: float,
            craters: int, crater_depth: float) -> np.ndarray:
    rng = np.random.default_rng(seed)
    shape = (size, size)
    field = fractal_noise(shape, octaves, rng, persistence=persistence)
    if craters > 0:
        radius_px = (CRATER_RADIUS_RANGE[0] * size, CRATER_RADIUS_RANGE[1] * size)
